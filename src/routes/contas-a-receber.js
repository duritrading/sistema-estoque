...
router.get('/', async (req, res) => {
    ...
    try {
        ...
        // ALTERAÇÃO PRINCIPAL: Adiciona filtro para mostrar apenas contas não vencidas
        const queryContas = `
            ...
            WHERE cr.data_vencimento >= $1 AND cr.data_vencimento <= $2
                AND cr.status != 'Pago'
                AND cr.data_vencimento >= $3  -- Apenas contas que ainda não venceram
            ORDER BY cr.data_vencimento ASC
        `;
        
        const [contasResult, categoriasResult] = await Promise.all([
            pool.query(queryContas, [data_inicio, data_fim, hoje]), // Adiciona o parâmetro 'hoje'
